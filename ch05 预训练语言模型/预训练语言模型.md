...
<div align=center>
<img src="./imgs/13.jpg" width="1000" height="650">
</div>
<div align=center>图13.Bert输入表示</div>


为了计算方便，在BERT中，这三种嵌入向量维度均为 $e$ ，因此可通过下式计算输入序列对应的输入表示 $v$：$v=v^t+v^s+v^p$。式中，$v^t$ 表示词嵌入，$v^s$表示块嵌入，$v^p$表示位置嵌入。三种嵌入向量的大小均为 $N*e$，$N$表示序列最大长度，$e$ 表示词嵌入维度。下面介绍这三种嵌入向量的计算方法：
- 词嵌入。与传统神经网络模型类似，Bert中的词嵌入同样通过词嵌入矩阵将输入文本转换成实值向量表示。假设输入序列 $x$ 对应的独热向量表示为 $e^t∈R^{N*|V|}$ ，其对应的词向量表示 $v^t$ 为：$v^t=e^t*W^t$，式中，$W^t∈R^{|V|*e}$ 表示可训练的词向量矩阵；$|V|$ 表示词表大小，$e$ 表示词向量维度。
- 块嵌入。块嵌入向量用来编码当前词属于哪一个块。输入序列中每个词对应的块编码为当前词所在块的序号，从0开始计数。当输入序列为单个块时(如单句文本分类)，所有词的块编码均为0；当输入序列为两个块时(如句对文本分类)，第一个句子中每个词对应的块编码为0，第二个句子中每个词对应的块编码为1。但是，$[CLS]$ 位(classification,表示序列开头的特殊词元)和第一个块结尾处的 $[SEP]$ 位(separator,表示序列分割的特殊词元)的块编码均为0。利用块向量矩阵 $W^s$ 将块编码 $e^s∈R^{N*|S|}$ 转换为实值向量，得到块向量 $v^s=e^s W^s$。式中，$W^s∈R^{|S|*e}$ 表示可训练的块向量矩阵；$|S|$ 表示块数量；$e$ 表示块向量维度。
- 位置嵌入。位置嵌入向量用来编码每个词的绝对位置。将输入序列中的每个词按照其下标顺序依次转换为位置独热编码。下一步，利用位置嵌入向量矩阵 $W^p$ 将位置独热编码 $e^p∈R^{N*N}$ 转换为实值向量，得到位置向量 $v^p=e^p W^p$。式中，$W^p∈R^{N*e}$ 表示可训练的位置向量矩阵；$N$ 表示最大位置长度；$e$ 表示位置向量维度。


为便于描述，输入表示层的操作统一归纳为 $X=[CLS]x_1^{(1)}x_2^{(1)}...x_n^{(1)}[SEP]x_1^{(2)}x_2^{(2)}...x_m^{(2)}[SEP]$，对于给定原始输入序列 $X$,经过如下处理得到Bert的输入表示 $v=inputrepresentation(X)$。其中，$v∈R^{N*e}$ 表示输入表示层的最终输出结果，即词嵌入、块嵌入和位置嵌入之和；$N$ 表示最大序列长度；$e$ 表示输入表示维度。


与GPT不同，Bert没有采用传统的基于自回归的语言建模方法，而是引入了基于自编码的预训练任务进行训练，基本预训练任务包括掩码语言模型和下一句预测组成。


#### 5.3.1 掩码语言模型


在提示学习一章中曾提及过掩码语言模型和下一句预测在预训练阶段可以提升上下文能力，本节对这两个概念进行更深入的介绍。传统基于条件概率建模的语言模型只能按顺序从左到右或按逆序从右到左建模文本序列，如果同时进行顺序建模文本和逆序建模文本，会导致信息泄露。为了实现文本的双向建模，即当前时刻的预测同时依赖于“历史”和“未来”，Bert采用了一种类似完型填空的做法，称为掩码语言模型(masked language model,MLM)。MLM预训练任务直接将输入的部分单词掩码，并通过深层transformer模型还原为原单词，从而避免了双向语言模型带来的信息泄露问题，迫使模型使用被掩码词周围的上下文信息还原掩码位置的词。在Bert中，采用了15%的掩码比例，即输入序列中15%的Wordpieces子词被掩码。当掩码时，模型使用 $[MASK]$ 标记替换原单词表示该位置已被掩码。然而这样会造成预训练阶段和下游任务精调阶段之间的不一致性，因为人为引入的 $[MASK]$ 标记并不会在实际的下游任务中出现。因此，为了缓解这个问题，当对输入序列进行掩码时，并非总是将其替换为 $[MASK]$ 标记，而会按概率选择以下三种操作中的一种：
- 以80%的概率替换为 $[MASK]$ 标记。
- 以10%的概率替换为词表中的任意一个随机词。
- 以10%的概率保持原词不变，即不替换。


在了解MLM预训练任务的基本方法后，接下来介绍其建模方法：
- 输入层。假设原始输入文本为 $x_1x_2...x_n$，通过上述方法掩码后的输入文本为 $x_1^{'}x_2^{'}...x_n^{'}$ ，$x_i$ 表示输入文本的第 $i$ 词，$x_i^{'}$ 表示经过掩码处理后的第 $i$ 个词。对掩码后的输人文本进行如下处理，得到Bert的输入表示 $v:X=[CLS]x_1^{'}x_2^{'}...x_n^{'}[SEP],v=inputrepresentation(X)$。如果输入文本的长度 $n$ 小于Bert的最大序列长度 $N$ ,需要将补齐标记 $[PAD]$ 拼接在输入文本后，直至达到Bert的最大序列长度 $N$。而如果输入序列 $X$ 的长度大于Bert的最大序列长度 $N$ ,需要对输入序列 $X$ 截断至Bert的最大序列长度 $N$。
- Bert掩码层。在Bert的输入表示 $v$ 经过 $L$ 层transformer，借助自注意力机制充分学习文本中的每个词之间的语义关联。$h^{[l]}∈R^{N*d}$ 表示第 $l$ 层transformer的隐含层输出，$h^{[l]}=transformer-block(h^{[l-1]}),$h^{[l]}∈R^{N*d}$ 表示第 $l$ 层transformer的输出，$h^{[0]}=v$。为便于描述，略去层与层之间的标记并简化为 $h=transformer(v)$，$h$ 表示最后一层transformer的输出，即 $h^{[l]}$。最终得到文本的上下文语义表示 $h∈R^{N*d}$ , $d$ 表示Bert的隐含层维度。
- 输出层。由于掩码语言模型仅对输入文本的部分词进行了掩码操作，因此并不需要预测输入文本中的每个位置，只需要预测已经掩码的位置。假设集合 $M={m_1,...,m_k}$ 表示所有掩码位置的下标，$k$ 表示总掩码数量。如果输入文本长度为 $n$ ，掩码比例为15%，则 $k= \lfloor n*0.15 \rfloor$。然后，以集合 $M$ 中的元素为下标，从输入序列的上下文语义表示 $h$ 中抽取出对应的表示，并将这些表示进行拼接得到掩码表示 $h^m∈R^{k*d}$。
...
- Bert的MLM任务对输入文本的部分单词随机掩码这一过程是在数据预处理阶段进行而非模型训练阶段，这就导致生成的掩码是静态的，同一个文本只有一种掩码模式，降低了训练数据的多样性和数据的复用效率。在Roberta中引入了动态掩码，即决定掩码位置和方法是在模型的训练阶段实时计算的。这样就能保证无论训练多少轮，都能够最大程度保证同一段文本能够在不同轮数下产生不同的掩码模式。
- RoBerta模型的原作者做了大量实验证明了不使用NSP损失的任务要略优于使用NSP损失的任务。


#### 5.3.7 ALbert


由于Bert这类模型的参数量较大，会占用大量计算资源，为解决该问题，研究人员提出了ALBert(a lite Bert)用以降低内存消耗和提高训练速度。ALBert主要使用了词向量参数因式分解和跨层参数共享，同时还引入了“句子顺序预测”预训练任务取代NSP任务。下面对这三个重要改动进行介绍。


##### 5.3.7.1 词向量因式分解


在Bert和RoBerta等模型里，词向量维度 $E$ 和transformer的隐含层维度 $H$ 是一样的。但这种设计存在两个问题：
- 从模型设计角度看，词向量的作用是将输入文本映射到上下文无关的静态表示中，即输入文本中的每个标记会独立地通过词向量矩阵映射到一个固定的向量，与其上下文无关。而大量的实验表明，Bert及其变种的预训练语言模型之所以强大，是因为词向量之上建立的深层transformer模型能够充分学习到每个标记的上下文信息。因此，transformer的最适合的隐含层维度 $H$ 要远大于词向量维度 $E$。
- 从实用角度看，词向量矩阵的参数量是词表大小 $V$ 乘以词向量维度 $E$。而通常词表大小 $V$ 较大，因为早期预训练语言模型中词向量维度 $E$ 和隐含层维度 $H$ 一样，当通过增大 $H$ 提升模型容量时，词向量维度 $E$ 也会随着增大，词向量矩阵的参数量也会随之上升。另外，词向量矩阵的更新是比较稀疏的，参数利用率不高。


因此，ALBert模型引入了词向量因式分解来解耦词向量维度 $E$ 和transformer隐含层维度 $H$。具体操作只需要令 $H\not =E$，但当 $H\not =E$ 时，词向量不能直接接入后续的多层transformer模型中。因此这里引入了一个全连接层，将词向量维度 $E$ 映射到transformer隐含层维度 $H$。引入词向量因式分解后，词向量部分的计算复杂度由 $O(VH)$ 降低到 $O(VE+EH)$。当transformer隐含层维度 $H$ 远大于词向量维度 $E$ 时，参数量的降幅尤为明显。


##### 5.3.7.2 跨层参数共享


在Bert中，多层transformer参数是不共享的，即每一层transformer都保留自己的参数。而在ALBert中，引入了跨层参数共享机制，使得每一层transformer的权重都是一样的。接下来通过一个三层transformer模型说明跨层参数共享，如下图所示。


<div align=center>
<img src="./imgs/14.jpg" width="1000" height="650">
</div>
<div align=center>图14.跨层参数共享</div>


ALBert采用了一种类似循环的结构，主体部分只包含一层transformer实体，通过循环操作，transformer的参数得到复用，并且可以实现深层计算(即循环多少次计算多少层)。


##### 5.3.7.3 句子顺序预测


ALBert引入了一个新的预训练任务--句子顺序预测(sentence order prediction,SOP)取代Bert的NSP任务。在SOP任务中，正例的构成与NSP一致，由相邻的两个文本片段构成，即构成“下一个句子”关系；而负例的构成则是直接对调两个文本片段的位置，而非NSP任务将第二段文本替换成随机文本片段。这样的目的是让模型学习到细微的语义差别及语篇连贯性，比NSP任务难度更大。


#### 5.3.8 MT-DNN


MT-DNN是一个采用BERT架构的算法，不同的是MT-DNN在下游任务中引入了多任务学习机制。它的网络结构采用的是多任务模型中经常用的策略，即在特征提取部分共享权值，在任务相关部分参数独立，如下图所示。


<div align=center>
<img src="./imgs/15.jpg" width="1000" height="650">
</div>
<div align=center>图15.MT-DNN网络结构</div>


MT-DNN共使用了4类自然语言理解任务，分别是：
- 单句分类任务（Single-sentence Classification Task，SST）：判断一个句子所属的类别；
- 文本相似度评分（Similarity Text Scoring， STS）：判断两个句子的相似性；
- 句子对关系分类（Pairwise Text Classification，PTC）：判断两个句子的关系，包括继承，冲突，中立等；
- 相关性排序（Relevance Ranking Task，RRT）：给定一个查询语句和若干个候选，判断查询与候选之间的相关性排序。


在MT-DNN的多任务中，分类任务使用交叉熵损失函数，回归任务使用最小均方误差作为目标值。


#### 5.3.8.1 MT-DNN预训练


MT-DNN的预训练采用了和BERT一致的结构和编码方式，包括基于Transformer的网络结构和编码方式以及预训练的任务。在MT-DNN中这一部分是被所有任务共享的，因此叫做共享层，共享层由词典编码器和Transformer编码器组成，经过它们得到的特征分别是上图中的 $l_1$ 和 $l_2$ 。


#### 5.3.8.2 单句分类


在单句分类中，对于一个给定的句子 $X$ ，它的标签值为 $c$。经过共享层得到特征 $l_2$ ，其中 $x$ 是 $l_2$中token为 $[cls]$ 的特征值，SST任务表示为：$P_r(c|X)=softmax(W^T_{SST}*x)$


#### 5.3.8.3 文本相似度评分


在MT-DNN中，两个句子 $(X_1,X_2)$ 会通过 $[sep]$ 标识切分后共同送入网络中，它们的相似度通过 $x$ 进行直接计算：$Sim(X_1,X_2)=W^T_{STS}*x$。


#### 5.3.8.4 句子对关系分类


给定前提 $G=(g_1,...,g_m)$ 和假设 $H=(h_1,...,h_n)$，句子对关系分类任务的目标是判断 $G$ 与 $H$ 的逻辑关系 $R$。MT-DNN的这个任务采用了SAN(stochastic answer network,随机答案网络)的网络结构，SAN的迭代计算过程如下图所示，它是一个多步推理模型。


<div align=center>
<img src="./imgs/16.jpg" width="1000" height="650">
</div>
<div align=center>图16.SAN网络结构</div>


模型首先使用共享层分别得出 $G$ 和 $H$ 的工作内存，分别表示为 $M^G∈R^{d*m}$ 和 $M^H∈R^{d*n}$。在这两个工作内存的基础上执行 $T$ 步推理，其中 $T$ 是一个超参数。在开始阶段，初始阶段，初始状态 $s_0$ 使用的是 $M^H$ 的加权和，如下式所示：$s_0=Σ_ja_jM_j^H,a_j=exp(w_1^T*M_j^H)/Σ_iexp(w_1^T*M_i^H)$。使用 $M^G$ 计算输入特征 $x_t$,其中 $t∈0,1,...,T-1$，如下式所示：$x_t=Σ_jβ_jM_j^G,β_j=softmax(s_{t-1}w^T_2M^G)$，使用GRU迭代更新 $s_t=GRU(s_{t-1},x_t)$，计算每个时间片预测的关系概率：$P_t=softmax(w_3^T[s_t;x_t;|s_t-x_t|;s_t*x_t])$。最终概率分布是所有时间片的均值：$P=avg([P_0,...,P_{T-1}])$。


#### 5.3.8.5 相关性排序


对于一个候选对 $(Q,A)$，相关度计算方式如式 $Rel(Q,A)=g(w^T_{RR}*x)$ 所示，损失函数如式 -$Σ_{Q,A^+}{exp[γRel(Q,A^+)]/Σ_{A'∈A}exp[γRel(Q,A')]}$ 所示。其中，$A$ 是候选假设的集合，由一个正样本 $A^+$ 和 $|A|-1$ 个负样本组成。


### 5.3.9 XLM


上面介绍的语言模型都是单语言的词向量模型，其它语言类型的词向量模型也是在这基础上换一个其它语言的语料库进行从头训练。这里要介绍的XLM是一个跨语言的词向量模型，它由基于单语料的无监督的任务和基于平行语料的有监督任务组成。


#### 5.3.9.1 跨语言共享字典


在XLM中，所有语言通过BPE共享一个字典。但是不同语言的语料库的大小差异非常大，传统的BPE会导致字典偏向于大语料库，导致低频语料库被切分成以词为单位。为了解决这个问题，作者提出了一个新的采样算法。假设一个数据集由 $N$ 个语言构成，那么这个数据集可以表示为 $\{C_i\}_{i=1,...,N}$。一个句子被采样的概率 $\{q_i\}_{i=1,...,N}$ 服从多项分布，表示为：$q_i=p_i^a/Σ_{j=1}^Np^a_j$ 且 $p_i=n_i/Σ_{k=1}^Nn_k$，其中 $n_i$ 是第 $i$ 个语言的样本数。$a$ 在在实验中的值是0.5。通过这个采样方法，低频样本会有更高的概率被采样到。


#### 5.3.9.2 因果语言模型


因果语言模型（Causal Language Model，CLM）是通过一个单词之前时间片的单词预测当前时间片的单词，表示为 $P(w_t|w_1,...,w_{t-1})$ 。它的思想和单向RNN语言模型一致，但是使用的是Transformer。


#### 5.3.9.3 XLM中的MLM


XLM中也使用了掩码语言模型，它和BERT的MLM有三点不同。
- XLM一个文本流并不限制句子的个数，直到他们的长度大于256后进行截断；
- 对于低频词，XLM使用了上采样的方法进行扩充；
- XLM的MLM在输入中加入了语言编码。它的结构如下图所示。


<div align=center>
<img src="./imgs/17.jpg" width="1000" height="250">
</div>
<div align=center>图17.XLM网络结构</div>


#### 5.3.9.4 翻译语言模型


为了训练XLM对平行语料的学习能力，XLM引入了翻译语言模型（Translation Language Model, TLM）。在TLM中，它的输入是拼接的平行语料，然后通过随机Mask掉任意语言的一部分来进行建模。通过TLM，XLM不仅学习了语言内部的相互关系，而且学会了语言间的对齐关系。和MLM一样，TLM的输入也加入了语言编码。


### 5.3.10 XLNet


语言模型通常被分为自回归语言模型和自编码语言模型两大类，两者的代表分别是GPT和Bert。自回归语言模型的优点是符合真实的建模场景，因为很多NLP任务都是从左向右的，缺点则是模型只能用到之前时间片的信息。Bert的优点是能够同时用到被预测单词的上文和下文，缺点主要是由于 $[mask]$ 的引入，导致训练阶段和微调阶段不一致的问题，因为在微调阶段是看不到 $[mask]$ 的。基于这些优缺点，研究人员提出了一种结合了AR和AE两个方法优点的模型XLNet。首先，XLNet可以通过最大化所有因式分解顺序的排列变换，学习双向语境信息。其次，XLNet使用自回归语言模型的特点，克服了BERT的训练和微调不一致的问题。下面首先对自回归语言模型和自编码语言模型这两个分类作简要介绍。


#### 5.3.10.1 自回归语言模型


自回归语言模型就是使用自回归模型对语言模型进行建模，自回归语言模型最常见的结构便是使用LSTM或者Transformer按照从左到右的顺序依次预测每个时间片的输出结果，例如GPT系列等。另外一种方式同时使用从左向右和从右向左的顺序进行自回归，其中典型的代表便是ELMo。ELMo虽然同时用到了上下文的信息，但其仍然是一个自回归语言模型，因为ELMo是分别进行了从左向右的自回归和从右向左的自回归，然后将这两个模型的隐层节点状态拼接到一起，在进行某一个语言模型建模时，并没用使用另外一个语言模型的特征，它其实就是自回归语言模型的拼接，并没用跳出自回归语言模型的范畴。


给定一个文本序列 $x={x_1,...,x_T}$，它可以通过正向的条件概率 $P(x)=\prod^T_{i=1}P(x_t|X_{<t})$ 或反向的条件概率 $P(x)=\prod^1_{t=T}P(x_t|X_{>t})$ 对这个序列进行建模。可见在进行单向语言模型的训练时，并不会用到另外一侧的文本信息，这对语言模型的效果是非常大的。一个前向的自回归语言模型通过最大化每个时间片预测结果的最大似然来完成语言模型的预训练：$max_θlogP_θ(X)=Σ_{t=1}^TlogP_θ(x_t|X_{<t})=Σ_{t=1}^Tlog(exp[h_θ(X_{1:t-1}^Te(x_t))]/Σ_{x'}exp[h_θ(X_{1:t-1}^Te(x'))])$，其中 $X_{<t}$ 表示所有时间片小于 $t$ 时间片的数据，即 $X_{1:t-1}$。$h_θ(X_{1:t-1})$ 是RNN或transformer，$e(x)$ 是序列 $x$ 的嵌入。


#### 5.3.10.2 自编码语言模型


自编码语言模型通过预测被掩码的单词来实现语言模型的训练，表示为：$max_θlogP_θ(\widetilde{X}|\widehat{X})≈Σ_{t=1}^Tm_tlogP_θ(x_t|\widehat{X})=Σ_{t=1}^Tm_tlog(exp[H_θ(\widehat{X})^T_te(x_t)]/Σ_{x'}exp[H_θ(\widehat{X})^T_te(x')])$ ，其中 $m_t=1$ 表示 $t$ 时刻是一个掩码，$H_θ$ 是一个transformer，它将长度为 $T$ 的序列编码成长度为 $T$ 的特征向量 $H_θ(X)=[H_θ(X)_1,...,H_θ(X)_T]$。自回归可以看到的时间片信息的范围是1到t-1，而自编码可以看到全部时间片的信息。在BERT中，句子中的每个单词会有15%的几率被替换成 $[mask]$ ，然后通过句子的其它部分来对当前时间片的 $[mask]$ 进行预测。可以看出，BERT本质上是一个去噪自编码器，而其中的 $[mask]$ 就相当于噪音数据。加入 $[mask]$ 之后确实可以比较方便的同时使用上下文信息，但是这个 $[mask]$ 符号在微调的时候是不存在的，导致了训练和微调的不一致问题，因此在上式训练中使用了 $≈$ 。


BERT的另外一个问题便是基于一条句子中被掩码掉的部分都是相互独立的，因为在预测一个时间片的内容时，其它被掩码掉的内容并不能提供有用的文本信息，所以这个假设并不成立。例如"积累原石"这句话，被掩码掉的字是'原'和'石'，句子变成了"积累[mask] [mask]"，当预测'原'时，我们并不知道另外一个被掩码掉的字是'石'，因此很难得到正确的预测结果。


综上，XLNet的出发点就是能否融合AR和AE的优点，创造一个既能看见上下文，又能保证训练和微调一致的模型。并且，XLNet的主体框架就是之前介绍过的transformer-xl模型，XLNet也由此得名。


#### 5.3.10.3 排列语言模型


XLNet的提出了排列语言模型（Permutation Language Model，PLM），PLM将AR和AE进行了融合，例如对于一个 $x_1\rightarrow x_2\rightarrow x_3\rightarrow x_4$ ，假设我们要预测的序列是 $x_3$，我们需要同时看到 $x_1,x_2,x_4$ ，这样才能解决自回归模型的不能同时看到上下文的问题。具体实现方式是，首先将输入序列进行打乱，再从中随机选择一个序列作为输入，比如选取到的排列为 $x_2\rightarrow x_4\rightarrow x_3\rightarrow x_1$，那么这个时候按照自回归模型的顺序依次对这个序列进行预测，在预测 $x_3$ 的时候就能同时看到它的上文 $x_2$ 和 $x_4$ 的信息了。所以PLM本质上则是一个先进行打乱，在从左向右依次预测的自回归语言模型，表示为: $max_θE_{z~Z_T}[Σ_{t=1}^TlogP_θ(x_{z_t}|X_{Z_{<t}})]$，其中， $Z_T$ 是长度为 $T$ 的序列的所有可能的排列，$z$ 是其中一个排列。$X_{Z_{<t}}$ 是该排列中所有介于1和t之间的序列，$logP_θ(x_{z_t}|X_{Z_{<t}})$ 表示为式 $P_θ(x_{z_t}=x|X_{Z<t})=exp[e(x)^Th(X_{Z_{<t}})]/Σ_{x'}exp[e(x')^Th(X_{Z_{<t}})]$ 。从式中可看出PLM的形式和AR是相同的，它做的就是从所有的排列中随机抽样一种，然后按照AR的顺序依次进行自回归。XLNet就是通过这种方式同时保证能够看到上下文且没有引入 $[mask]$ 标记的。


这种全排列的方式破坏了序列 $x$ 原始的排列顺序，如果不加入位置编码的话，模型是无法根据要预测时间片的真实位置进行预测的，对于一个句子的两个排列 $z^{(1)}$ 和 $z^{(2)}$ ,假设这个句子满足下式，例如句子“原神启动”的两个排列“原启动神”和“原启神动”：$z_{<t}^{(1)}=z_{<t}^{(2)}=z_{<t},z_t^{(1)}=i\ne j=z_t^{(2)}$ ，其中 $i、j$是要预测的单词在句子中的实际位置。根据自编码语言模型的训练公式，这两个时间片的似然概率如下式所示：$P_θ(x_i=x|X_{Z_{<t}})_{z_t^{(1)}=i,z_{<t}^{(1)}=z_{<t}}=P_θ(x_j=x|X_{Z_{<t}})_{z_t^{(1)}=j,z_{<t}^{(2)}=z_{<t}}=exp[e(x)^Th(X_{Z<t})]/Σ_{x'}exp[e(x')^Th(X_{Z<t})]$。从上式可看出，不同目标位置的 $i$ 和 $j$ 的预测结果是完全一致的，但是很明显它们的标签值是不相同的。为了避免这个问题，XLNet中加入了目标位置编码 $z_t$ ，具体计算方式如下：$P_θ(x_{z_t}=x|X_{Z<t})=exp[e(x)^Tg_θ(X_{Z_{<t}},z_t)]/Σ_{x'}exp[e(x')^Tg_θ(X_{Z_{<t}},z_t)]$，其中 $g_θ(X_{Z_{<t}},z_t)$ 表示加入了位置编码的一种新编码方式，称为双流自注意力，下文将予以介绍。


上面的讲解过程中，虽然是按照先全排列再随机抽取的策略（相当于把原始输入随机打乱），但是我们在微调的时候，我们并不能将句子随机打乱后再输入到模型中。所以在训练的时候，输入还必须保持句子原始的输入顺序，然后在网络中做一些操作，来实现随机打乱的功能。具体的讲，XLNet采用了Attention掩码的模块来实现对句子的打乱，如下图所示，首先生成序列 $x_1\rightarrow x_2\rightarrow x_3\rightarrow x_4$ 的一个随机序列，如 $x_3\rightarrow x_2\rightarrow x_4\rightarrow x_1$，然后根据这个序列生成mask掩码，那么将掩码和原始序列相乘后便可以得到前面介绍的PLM的功能，例如在预测4时，1被mask掉，2和3可见。


<div align=center>
<img src="./imgs/18.jpg" width="500" height="300">
</div>
<div align=center>图18.XLNet注意力掩码</div>


#### 5.3.10.4 双流自注意力


上文引入了双流自注意力 $g_θ(X_{Z_{<t}},z_t)$。根据上面的介绍，$g_θ(X_{Z_{<t}},z_t)$ 应当满足两个特性：
- 为了预测 $x_{z_t}，g_θ(X_{Z_{<t}},z_t)$ 只能使用位置信息 $z_t$ 而不能使用内容信息 $x_{z_t}$，因为当知道了 $t$ 时刻的内容时，再对它进行预测就没有意义了；
- 为了预测 $z_t$ 之后的单词，$g_θ(X_{Z_{<t}},z_t)$ 必须编码了 $x_{z_t}$，这样才能保证之后的时间片预测可以使用上文的信息。


这种特性在之前介绍的Transformer-XL中是互相矛盾的，因此作者引入了双流自注意力。双流自注意力由内容流和查询流组成，单看内容流 $h_θ(x_{Z_{\le t}})$，如下左图所示，就是一个传统的Transformer(-XL)，因为它同时使用位置编码和内容编码，表示为：$h_{Z_t}^{(m)}\leftarrow attention(Q=h_{Z_t}^{(m-1)},KV=h_{Z_{\le t}^{(m-1)}};θ)$，其中 $m$ 表示自注意力的层数，$m=1,...,M$。在编码 $h_1^{(0)}$ 时，键和值会用到 $h_1^{0}、h_2^{0}、h_3^{0}、h_4^{0}$。


<div align=center>
<img src="./imgs/19.jpg" width="1000" height="250">
</div>
<div align=center>图19.XLNet内容流</div>


另外一条分支是查询流 $g_θ(x_{z_t})$，如上右图所示。在计算查询时，只会用到之前时间片 $h_2^{0}、h_3^{0}、h_4^{0}$ 的内容信息，因此 $Q=g_1^{(0)}$ 查询流起到了和BERT中掩码操作类似的功能，因为XLNet抛弃了掩码操作，于是在查询流中，它直接忽略了当前预测时间片的实际内容，而只保留了这个时间片的位置信息，如下式所示：$g_{z_t}^{(m)}\leftarrow attention(Q=g_{Z_t}^{(m-1)},KV=h_{Z_{<t}^{(m-1)}};θ)$。


根据上面式子可以看出，内容流和查询流在计算时，分别使用的是他们自己的上一个时间片的内容向量 $h_{z_t}^{(m-1)}$ 和查询向量 $g_{z_t}^{(m-1)}$，但是在计算Key和Value时，则是使用了内容向量 $h_{Z_{\le t}^{(m-1)}}$，这也就保证了特征2。但是在计算查询流 $g_{z_t}^{(m)}$ 时，Key和Value使用的是 $h_{Z_{<t}^{(m-1)}}$，保证了查询流不能看到 $z_t$ 的内容，因此保证了特性1。


在初始化的时候，查询流的值初始化为一个可以训练的向量，即 $g_i^{(0)}=w$，内容流使用单词对应的嵌入向量进行初始化：$h_i^{(0)}=e(x_i)$。


在微调的时候，XLNet会将查询流去掉而只用内容流，最后在计算公式的时候再用最上面的 $Q$ 向量 $g_{Z_t}^{(M)}$。


#### 5.3.10.5 部分预测


如果按照上面介绍的方法从头开始预测一个句子时，在没有上下文的情况下，前几个时间片的内容基本上式不可能预测的，这使得训练很难收敛。为了解决这个问题，在训练时，XLNet中只对部分句子进行预测。具体的讲，XLNet中引入了一个分隔符 $c$ ，对于一个随机打乱的排列 $Z$，它被分成非目标子序列 $Z_{\le c}$ 和目标子序列 $Z_{>c}$。此时优化目标变成了根据非目标子序列预测目标子序列的条件概率模型，表示为：$max_θ E_{z~Z_T}[logP_θ(X_{Z_{>c}|X_{Z_{\le c}}})]=E_{z~Z_T}[Σ_{t=c+1}^{|z|}logP_θ(x_{z_t}|X_{Z_{<t}})]$。


XLNet设置了一个超参数 $K$，一个句子中的 $1/K$ 个单词是需要被预测的，即：$|z|/(|z-c|)≈K$,实验结果表明最佳的 $K$ 值介于6和7之间，由此确定分隔符的位置。


#### 5.3.10.6 回顾transformer-xl


作者选择了Transformer-XL作为基础框架。对于相对位置编码来说，XLNet使用原始输入的位置编码，因为随机排列之后的位置编码是没有意义的。因为Transformer-XL的分段自回归机制将输入序列分成若干个片段，然后递归式的将前一个片段的节点特征提供给当前片段，那么XLNet如何与片段自回归机制进行配合的呢?举例来说，假设一个长序列 $s$ 被分成两个片段 $\widetilde{x}=s_{1:T}$ 和 $x=s_{T+1:2T}$，$\widetilde{z}$ 和 $z$ 是两个随机打乱的序列，在计算当前时间片的特征编码时，它使用的是上一个时间片这个片段的特征 $h_{Z \le t}^{(m-1)}$ 和当前时间片前一个片段的特征 $\widetilde{h}^{(m-1)}$：$h_{Z_t}^{(m)}\leftarrow attention(Q=h_{Z_t}^{(m-1)},KV=[\widetilde{h}^{(m-1)},h_{Z \le t}^{(m-1)}];θ)$，其中 $[\cdot;\cdot]$ 表示拼接操作。